...
class Gym :
    ...
    def updateMember(self) :
        print ( '\nGym update the records of members.\n' )
                ...
                if updatation == items :
                    ...
                    if choose == '1' :
                        ...
                        def newNo() :
                            try :
                                new_val = int ( input ( '\nNew Mob. No.: ' ) )
                                if int ( int ( new_val ) / 1000000000 ) > 0 and int (
                                        int ( new_val ) / 1000000000 ) < 10 :
                                    group[0] = new_val
                                    print ( '\nRecord Successfully modified.' )
                                else :
        ...
        try :
            ...
            if int ( int ( new_reg ) / 1000000000 ) > 0 and int ( int ( new_reg ) / 1000000000 ) < 10 :
                for group in self.cusInfo :
                    for items in group :
                        if new_reg == items :
                            regimen = self.regChart ( group[5] )  # bmi is extracted and pass to create regimen data
